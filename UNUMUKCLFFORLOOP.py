...
def model_lstm_SR_Words(max_no_threads, actual_threads, max_features,max_classes, maxlen, embed_size, embedding_matrix,bs):
    ...
    symbolic_shape=K.shape(inp_recv_t)
    # initalize SLTM state
    # import keras.backend as K
    f_h_s = K.variable(K.random_normal(shape=[128]))
    f_c_s = K.variable(K.random_normal(shape=[128]))
    b_h_s = K.variable(K.random_normal(shape=[128]))
    b_c_s = K.variable(K.random_normal(shape=[128]))
    #h_r=Lambda(lambda x:x)(h_r)
    #c_r=Lambda(lambda x:x)(c_r)
    x_r = K.placeholder(shape=[None,maxlen,256])
    x_s = K.placeholder(shape=[None,maxlen,256])
    print("shape after init", x_r.shape,f_h_s.shape)
    for input_t in range(actual_threads):
        ...
        x_r, f_h_r ,f_c_r,b_h_r,b_c_r= Bidirectional(LSTM(128, return_state=True,return_sequences=True))(inpr)
        #,initial_state=[f_h_s,f_c_s,b_h_s,b_c_s])
        print("shape after  first bilstm", x_r.shape,f_h_r.shape,f_c_r.shape)
    ...
    #concatenate if you need to merge states to feed into LSTM but dimension of lSTM need to 2*LSTMsize
        x_s, f_h_s ,f_c_s,b_h_s,b_c_s= Bidirectional(LSTM(128,  return_state=True,return_sequences=True))(inps, initial_state=[f_h_r,f_c_r,b_h_r,b_c_r])
    ...
    # h_s=Dense(64)(h_s)
    # c_s = Dense(64)(c_s)
    x_r4, f_h_r4, f_c_r4, b_h_r4, b_c_r4 = Bidirectional(LSTM(64, return_state=True, return_sequences=False))(inpr,initial_state=[f_h_s3,f_c_s3,b_h_s3, b_c_s3])
    # ,initial_state=[f_h_s,f_c_s,b_h_s,b_c_s])
    x_s4, f_h_s4, f_c_s4, b_h_s4, b_c_s4 = Bidirectional(LSTM(64, return_state=True, return_sequences=True))(inps,initial_state=[f_h_r4,f_c_r4,b_h_r4,b_c_r4])
    print("fourth layer output", x_r4.shape, x_s4.shape)
    '''
    out_x=Concatenate(axis=1)([x_r,x_s])
...
df_L=pd.read_csv(fotrain,converters={'received':ast.literal_eval,'sent':ast.literal_eval})
