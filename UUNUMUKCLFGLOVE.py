...
def model_lstm_SR_Words(max_no_threads, actual_threads, max_features,max_classes, maxlen, embed_size, embedding_matrix,bs):
    ...
    '''
    out_x=Concatenate(axis=1)([x_r1,x_s1,x_r2])
...
def filternounsandverbs(sent):
    allowed_postags = ['NOUN','ADJ','ADV', 'AUX', 'ADP','VERB']
...
#path for saving glove embedding of training data
foembed=os.path.join("d:/USHUR/PythonProg/DATAREP/UNUMUK","glove.dat")
...
def get_glove_embedding_matrix(embedding_matrix,word_index,embeddings_index, gb2us,e_f):
    for word, i in word_index.items():
        embedding_vector = embeddings_index.get(spell.correction(lm.lemmatize(word.lower())))
        ...
        # words not found in embedding index will be all-zeros.
            ...
            try:
                gb2us[word.lower()]
                embedding_vector = embeddings_index.get(spell.correction(lm.lemmatize(gb2us[word.lower])))
                if embedding_vector is not None:
                    embedding_matrix[i] = embedding_vector
                    e_f+=1
                continue
...
#embedding_matrix,e_f=get_glove_embedding_matrix(embedding_matrix,word_index,embeddings_index,gb2us,e_f)
...
#Y_train is a dataframe, so convert into array
...
class_weights = class_weight.compute_class_weight('balanced',
                                                 cl,
                                                 y_ints)
history = model.fit([X_R_train, X_S_train],Y_train,class_weight=class_weights,epochs=epochs,batch_size=batch_size,validation_split=0.2,
